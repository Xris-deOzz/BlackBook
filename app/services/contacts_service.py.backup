...
class ContactsService:
        ...
        try:
            ...
            while True:
                results = service.people().connections().list(
                    ...
                    pageSize=1000,
                    personFields="names,emailAddresses,phoneNumbers,photos,organizations,metadata,birthdays,biographies,addresses,memberships",
                    pageToken=page_token,
                ).execute()
    ...
    def _match_contact_to_person(self, contact: GoogleContact) -> Person | None:
        """Try to find an existing person by matching email addresses."""
    ...
    def _assign_tags_to_person(self, person: Person, labels: list[str]) -> bool:
        ...
        assigned_any = False
        existing_tag_ids = {tag.id for tag in person.tags}

        for label_name in labels:
            tag = self._get_or_create_tag(label_name)
            if tag.id not in existing_tag_ids:
                # Create PersonTag link
